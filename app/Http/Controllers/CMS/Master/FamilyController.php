...
class FamilyController extends Controller
{
    ...
    {
        return view('cms.master.family-index');
    }

    public function getDataAjax()
    {
        $kingdoms = Family::select('id', 'name')->get();
    ...
    public function update(Request $request)
    {
        ...
        $check = Family::where('name', $taxonimic)->count();

        if ($check >= 1) {
            $request->session()->flash('error', 'Data sudah ada!');
        } else {$check = Family::where('id', $id)->update(['name' => $taxonimic]);
